...
_TEST_WITH_AI_LOG_MAX_BYTES = 5 * 1024 * 1024  # 5 MiB
_TEST_WITH_AI_LOG_BACKUP_COUNT = 3
...
def setup_logger(level: int | str = logging.INFO):
    """Configure logging to only output from this package (test_with_ai), not deps."""
    ...
    # Only attach handler to our namespace so only test_with_ai.* logs are printed.
...
def add_test_with_ai_file_handler(log_path: Path) -> None:
    """Add a RotatingFileHandler to the test_with_ai logger for /daemon logs.
    ...
    Args:
        log_path: Path to the log file (e.g. WORKING_DIR / "test_with_ai.log").
    """
    ...
    file_handler = logging.handlers.RotatingFileHandler(
        log_path,
        encoding="utf-8",
        maxBytes=_TEST_WITH_AI_LOG_MAX_BYTES,
        backupCount=_TEST_WITH_AI_LOG_BACKUP_COUNT,
    )
