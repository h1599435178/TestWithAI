...
This package provides hook implementations for Test with AIAgent that follow
...
Example:
    >>> from test_with_ai.agents.hooks import BootstrapHook, MemoryCompactionHook
    >>> from pathlib import Path
    >>>
    >>> # Create hooks (they are callables following AgentScope's interface)
    >>> bootstrap = BootstrapHook(Path("~/.test_with_ai"), language="zh")
