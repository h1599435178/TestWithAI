...
description: "使用可视浏览器自动完-Test with AI 的钉钉频道接入。适用于用户提到钉钉、DingTalk、开发者后台、Client ID、Client Secret、机器人、Stream 模式、绑定或配置 channel 的场景；支持遇到登录页时暂停，等待用户登录后继续-
metadata:
  {
    "test_with_ai":
      {
...
-skill 用于通过可视浏览器自动化完成钉钉应用创建-Test with AI channel 绑定-
...
2. 遇到登录关卡必须暂停-
   - 若页面出现登录界面（-`登录`、扫码登录、手机号/密码登录），立即停止自动操作-
   - 明确提示用户先手动登录，再等待用户回复“登录好-继续”-
   - 未收到用户确认前，不得继续执行后续步骤-
...
2. 明确告知图片规范（显著提示）-
   - 机器人图标：仅支-JPG/PNG，`240*240px` 以上，`1:1`，`2MB` 以内，无圆角-
   - 机器人消息预览图：格-`png/jpeg/jpg`，不超过 `2MB`-
...
1. 若用户提供本地路径，直接用于上传-
2. 若用户提供图-link，先下载到本地临时文件，再执行上传-
3. 上传动作顺序必须是：
   - 先点击页面上传入口（触发 chooser-
   - 再调-`file_upload` 传入本地路径数组（`paths_json`-
...
1. `file_upload` -`paths_json` 必须是“JSON 字符串数组”，注意转义-
...
2. 若页面在 iframe 内，建议优先带上 `frame_selector`，否则可能出现找不到上传控件-chooser 未触发-
...
4. 机器人图标区域的常见结构特征可用于定位（示例）：
   - `text: "* 机器人图-`
   - `button: "使用应用图标"`
   - `button: "avatar"`（通常内部-`img "avatar"`-

5. -snapshot 中同时出现“使用应用图标”和“avatar”时，优先点-`avatar` 按钮触发上传，再执行 `file_upload`-
...
1. 进入创建路径-
   - 应用开-> 企业内部应用 -> 钉钉应用 -> 创建应用
2. 填写应用信息（优先使用用户自定义，否则使用默认值）-
   - 应用名称：默-`Test with AI`
   - 应用描述：默-`Your personal assistant`
3. 保存并创建应-
...
1. 点击**应用能力**中的**添加应用能力**，找-*机器-*并添-
2. -*机器人配-*右侧的switch按钮切换为打开
3. 填写**机器人名-*-*机器人简-*-*机器人描-*
4. 上传**机器人图-*（用户自定义或默认图）：
   - 点击机器人图标下面的图片
...
5. 上传**机器人消息预览图**（用户自定义或默认图）：
   ...
   - 若为 link，先下载到本地再上传
   - 若提示图片不合规，暂停并让用户手动上传合规图片后继续
6. 确认消息接收模式设置-`Stream 模式`
7. 选择**发布**，此时会有进一步弹出的确认页面，选择发布。注意：**一定要发布机器-*之后再进行下一-
...
5. 只有看到发布成功状态，才可继续执行后续步骤/给用户“已生效”结-

### 步骤 5：获取凭-

1. 打开 `基础信息 -> 凭证与基础信息`
2. 告知用户`Client ID`（AppKey）和`Client Secret`（AppSecret）在该页面上。不主动进行修改，引导用户自行绑-
...
1. 控制台前端配置：
   - Test with AI console 中进-`控制 -> 频道 -> DingTalk`
...
路径：`~/.test_with_ai/config.json`，位-`channels.dingtalk` 下-

### 凭证交付要求（强制）

1. agent 只负责引导用户进入凭证页、获取并展示 `Client ID` 与真-`Client Secret`-
2. agent 不主动改 `console` 配置、不主动-`~/.test_with_ai/config.json`-
3. 必须提示用户按以下两种方式之一手动填写-
   - 控制台前端：`控制 -> 频道 -> DingTalk`
   - 配置文件：编-`~/.test_with_ai/config.json` -`channels.dingtalk` 字段
...
- 优先使用最-`snapshot` -`ref`；仅在必要时使用 `selector`-
