...
_USE_SYNC_PLAYWRIGHT = (
    sys.platform == "win32" and os.environ.get("TEST_WITH_AI_RELOAD_MODE") == "1"
)
...
def _ensure_playwright_async():
    ...
    try:
        ...
        raise ImportError(
            "Playwright not installed. Use the same Python that runs Test with AI (e.g. "
...
def _sync_browser_launch(headless: bool):
    ...
    use_default = not is_running_in_container() and os.environ.get(
        "TEST_WITH_AI_BROWSER_USE_DEFAULT",
        "1",
    ...
    try:
        if _USE_SYNC_PLAYWRIGHT:
            ...
            if exe:
                ...
                # macOS: default Safari or no Chromium â†?use WebKit (Safari)
                pw_browser = await pw.webkit.launch(
                    headless=_state["headless"],
                )
            else:
                # Windows/Linux without system Chromium â†?Playwright's Chromium
