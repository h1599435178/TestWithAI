...
WORKING_DIR = (
    Path(os.environ.get("TEST_WITH_AI_WORKING_DIR", "~/.test_with_ai"))
    .expanduser()
    .resolve()
)
SECRET_DIR = (
...
JOBS_FILE = os.environ.get("TEST_WITH_AI_JOBS_FILE", "jobs.json")

CHATS_FILE = os.environ.get("TEST_WITH_AI_CHATS_FILE", "chats.json")

CONFIG_FILE = os.environ.get("TEST_WITH_AI_CONFIG_FILE", "config.json")

HEARTBEAT_FILE = os.environ.get("TEST_WITH_AI_HEARTBEAT_FILE", "HEARTBEAT.md")
...
LOG_LEVEL_ENV = "TEST_WITH_AI_LOG_LEVEL"

# Env to indicate running inside a container (e.g. Docker). Set to 1/true/yes.
RUNNING_IN_CONTAINER = os.environ.get("TEST_WITH_AI_RUNNING_IN_CONTAINER", "false")
...
try:
    MODEL_PROVIDER_CHECK_TIMEOUT = float(
        os.environ.get("TEST_WITH_AI_MODEL_PROVIDER_CHECK_TIMEOUT", "5.0"),
    )
...
DOCS_ENABLED = os.environ.get("TEST_WITH_AI_OPENAPI_DOCS", "false").lower() in (
    "true",
    "1",
    "yes",
)
...
# Custom channel modules (installed via `test_with_ai channels install`); manager
...
MEMORY_COMPACT_KEEP_RECENT = int(
    os.environ.get("TEST_WITH_AI_MEMORY_COMPACT_KEEP_RECENT", "3"),
)

MEMORY_COMPACT_RATIO = float(
    os.environ.get("TEST_WITH_AI_MEMORY_COMPACT_RATIO", "0.7"),
)
...
# CORS configuration â€?comma-separated list of allowed origins for dev mode.
# Example: TEST_WITH_AI_CORS_ORIGINS="http://localhost:5173,http://127.0.0.1:5173"
# When unset, CORS middleware is not applied.
CORS_ORIGINS = os.environ.get("TEST_WITH_AI_CORS_ORIGINS", "").strip()
