...
Shared by in-chat /daemon <sub> and CLI `test_with_ai daemon <sub>`.
Logs: tail WORKING_DIR / "test_with_ai.log". Restart: in-process reload of channels,
...
async def run_daemon_restart(context: DaemonContext) -> str:
    ...
    return (
        ...
        "Run the app (e.g. `test_with_ai app`) and use /daemon restart in chat, "
...
def run_daemon_version(context: DaemonContext) -> str:
    ...
    return (
        ...
        f"- Log file: {context.working_dir / 'test_with_ai.log'}"
    )


def run_daemon_logs(context: DaemonContext, lines: int = 100) -> str:
    """Tail last N lines from WORKING_DIR / test_with_ai.log."""
    log_path = context.working_dir / "test_with_ai.log"
