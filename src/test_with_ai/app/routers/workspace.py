...
async def download_workspace():
    ...
    filename = f"test_with_ai_workspace_{timestamp}.zip"
...
@router.post(
    ...
    description=(
        ...
        "are left unchanged (e.g. test_with_ai.db, runtime dirs).  Download packs "
...
async def upload_workspace(  # pylint: disable=too-many-branches
    ...
    tmp_dir = None
    try:
        tmp_dir = Path(tempfile.mkdtemp(prefix="test_with_ai_upload_"))
        with zipfile.ZipFile(io.BytesIO(data)) as zf:
