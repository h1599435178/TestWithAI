...
from ..utils.logging import setup_logger, add_test_with_ai_file_handler
...
async def lifespan(
    ...
    add_test_with_ai_file_handler(WORKING_DIR / "test_with_ai.log")
    await runner.start()
...
# Console static dir: env, or test_with_ai package data (console), or cwd.
_CONSOLE_STATIC_ENV = "TEST_WITH_AI_CONSOLE_STATIC_DIR"


def _resolve_console_static_dir() -> str:
    if os.environ.get(_CONSOLE_STATIC_ENV):
        return os.environ[_CONSOLE_STATIC_ENV]
    # Shipped dist lives in test_with_ai package as static data (not a Python pkg).
    pkg_dir = Path(__file__).resolve().parent.parent
    candidate = pkg_dir / "console"
    if candidate.is_dir() and (candidate / "index.html").exists():
        return str(candidate)
    # the following code can be removed after next release,
    # because the console will be output to test_with_ai's
    # `src/test_with_ai/console/` directory directly by vite.
    cwd = Path(os.getcwd())
...
if os.path.isdir(_CONSOLE_STATIC_DIR):
    ...
    @app.get("/test_with_ai-symbol.svg")
    def _console_icon():
        f = _console_path / "test_with_ai-symbol.svg"
        if f.is_file():
