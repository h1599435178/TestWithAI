...
class LocalModelManager:
    ...
    def _download_from_huggingface(
        ...
        try:
            ...
            raise ImportError(
                ...
                "Install it with: pip install 'test_with_ai[local]'",
            ) from e

        _ensure_models_dir()
        local_dir = MODELS_DIR / _sanitize_repo_id(repo_id)

        # MLX models are directory-based â€?download entire repo
    ...
    def _validate_mlx_directory(model_dir: Path) -> None:
        ...
        if missing:
            raise RuntimeError(
                f"MLX model download appears incomplete â€?missing files "
        ...
        if not st_files:
            raise RuntimeError(
                f"MLX model download appears incomplete â€?no .safetensors "
