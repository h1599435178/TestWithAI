...
def _select_provider_interactive(
    ...
    *,
    ...
    for d in all_providers:
        mark = "âœ? if data.is_configured(d) else "âœ?
...
def configure_provider_api_key_interactive(
    ...
    if defn is None:
        ...
        click.echo(
            ...
            f"  test_with_ai models add-provider {provider_id} "
            f'-n "My Provider"',
        )
        raise SystemExit(1)
    ...
    if defn.is_local:
        ...
        click.echo(
            "Use 'test_with_ai models download' to add models, "
            "then 'test_with_ai models set-llm' to activate.",
        )
        return provider_id
    ...
    click.echo(
        f"âœ?{defn.name} â€?API Key: {mask_api_key(api_key) or '(not set)'}"
...
def _add_models_interactive(provider_id: str) -> None:
    ...
    while click.confirm("Add a model?", default=not all_models):
        ...
        try:
            ...
            click.echo(f"âœ?Model '{model_name}' ({model_id}) added.")
...
def configure_llm_slot_interactive(*, use_defaults: bool = False) -> None:
    ...
    if not eligible:
        if use_defaults:
            click.echo(
                "No LLM provider configured. Run 'test_with_ai models config' "
                "to configure later.",
            )
            return
        click.echo(
    ...
    if not model and use_defaults:
        click.echo(
            ...
            "Run 'test_with_ai models config' to set one.",
        )
        return
    set_active_llm(pid, model)
    click.echo(f"âœ?LLM: {defn.name} / {model}")


def configure_providers_interactive(*, use_defaults: bool = False) -> None:
    """Full interactive setup: configure provider â†?add models â†?
    activate LLM."""
    if use_defaults:
    ...
    for defn in list_providers():
        ...
        if defn.is_local:
            ...
            if all_models:
                ...
                click.echo("  Use 'test_with_ai models download' to add models.")
        else:
            if defn.is_custom or not defn.default_base_url:
...
def add_provider_cmd(
    ...
    click.echo(f"âœ?Custom provider '{name}' ({provider_id}) created.")
    if base_url:
        click.echo(f"  base_url: {base_url}")
    click.echo(
        "  Run 'test_with_ai models add-model' to add models, "
        "then 'test_with_ai models config-key' to set the API key.",
    )
...
def remove_provider_cmd(provider_id: str, yes: bool) -> None:
    ...
    try:
        ...
        click.echo(click.style(f"Error: {exc}", fg="red"))
        raise SystemExit(1) from exc
    click.echo(f"âœ?Custom provider '{provider_id}' deleted.")
...
def add_model_cmd(provider_id: str, model_id: str, model_name: str) -> None:
    ...
    try:
        ...
        raise SystemExit(1) from exc
    click.echo(
        f"âœ?Model '{model_name}' ({model_id}) added to '{provider_id}'.",
    )
...
def remove_model_cmd(provider_id: str, model_id: str) -> None:
    ...
    try:
        ...
        click.echo(click.style(f"Error: {exc}", fg="red"))
        raise SystemExit(1) from exc
    click.echo(f"âœ?Model '{model_id}' removed from '{provider_id}'.")
...
def download_cmd(
    ...
    \b
    Examples:
      test_with_ai models download TheBloke/Mistral-7B-Instruct-v0.2-GGUF
      test_with_ai models download TheBloke/Mistral-7B-Instruct-v0.2-GGUF \\
          -f mistral-7b-instruct-v0.2.Q4_K_M.gguf
      test_with_ai models download Qwen/Qwen2-0.5B-Instruct-GGUF --source modelscope
    """
    try:
        ...
        click.echo(
            click.style(
                ...
                "Install with: pip install 'test_with_ai[local]'",
                fg="red",
            ),
        )
        raise SystemExit(1) from exc
    ...
    click.echo(
        ...
        f"  test_with_ai models set-llm  (select '{backend}' provider)",
    )
...
def list_local_cmd(backend: str | None) -> None:
    ...
    if not models:
        ...
        click.echo("Use 'test_with_ai models download <repo_id>' to download one.")
        return
...
def ollama_pull_cmd(model_name: str) -> None:
    ...
    \b
    Examples:
      test_with_ai models ollama-pull mistral:7b
      test_with_ai models ollama-pull qwen2.5:3b
    """
    ...
    try:
        ...
        click.echo(f"âœ?Model '{model_name}' downloaded successfully.")
        click.echo("\nTo use this model, run:\n  test_with_ai models set-llm")
    except ImportError as exc:
        click.echo(
            click.style(
                str(exc),
...
def ollama_list_cmd() -> None:
    ...
    if not models:
        ...
        click.echo("Use 'test_with_ai models ollama-pull <model>' to download one.")
        return
...
def ollama_remove_cmd(model_name: str, yes: bool) -> None:
    ...
    \b
    Examples:
      test_with_ai models ollama-remove mistral:7b
      test_with_ai models ollama-remove qwen2.5:3b -y
    """
    from ..providers.ollama_manager import OllamaModelManager
    from ..providers.store import get_ollama_host
    ...
    try:
        ...
        click.echo(f"âœ?Model '{model_name}' deleted.")
    except ImportError as exc:
        click.echo(
            click.style(
                str(exc),
