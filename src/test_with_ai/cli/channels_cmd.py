...
# Template for `test_with_ai channels install <key>` stub (channel key substituted).
...
from test_with_ai.app.channels.base import BaseChannel
from test_with_ai.app.channels.schema import ChannelType
...
def configure_voice(
    ...
    welcome_greeting = click.prompt(
        ...
        or "Hi! This is Test with AI. How can I help you?",
        type=str,
    )
...
def configure_channels_interactive(config: Config) -> None:
    ...
    while True:
        ...
        for channel_key, (channel_name, _) in configurators.items():
            ...
            status = "enabled" if _channel_enabled(channel_config) else "disabled"
...
def list_cmd() -> None:
    ...
    if not config_path.is_file():
        ...
        click.echo("Run `test_with_ai channels config` to create one.")
        cfg = load_config()
    else:
