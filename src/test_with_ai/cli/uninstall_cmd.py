# -*- coding: utf-8 -*-
"""test_with_ai uninstall â€?remove the Test with AI environment and CLI wrapper."""
...
def _remove_path_entry(profile: Path) -> bool:
    """
    Remove Test with AI PATH lines from a shell profile. Returns True if changed.
    """
    if not profile.is_file():
        return False

    text = profile.read_text()
    # Remove the "# Test with AI" comment line and the export PATH line
    cleaned = re.sub(
        r"\n?# Test with AI\nexport PATH=\"\$HOME/\.test_with_ai/bin:\$PATH\"\n?",
        "\n",
        text,
    )
    if cleaned == text:
...
def uninstall_cmd(purge: bool, yes: bool) -> None:
    """Remove Test with AI environment, CLI wrapper, and shell PATH entries."""
    wd = WORKING_DIR

    if purge:
        click.echo(f"This will remove ALL Test with AI data in {wd}")
    else:
        click.echo(
            "This will remove the Test with AI Python environment and CLI wrapper.",
        )
    ...
    click.echo("")
    click.echo("Test with AI uninstalled. Please restart your terminal.")
