# -*- coding: utf-8 -*-
"""test_with_ai uninstall ->remove the TestWithAI environment and CLI wrapper."""
...
def _remove_path_entry(profile: Path) -> bool:
    """
    Remove TestWithAI PATH lines from a shell profile. Returns True if changed.
    """
    if not profile.is_file():
        return False

    text = profile.read_text()
    # Remove the "# TestWithAI" comment line and the export PATH line
    cleaned = re.sub(
...
def uninstall_cmd(purge: bool, yes: bool) -> None:
    """Remove TestWithAI environment, CLI wrapper, and shell PATH entries."""
    wd = WORKING_DIR

    if purge:
        click.echo(f"This will remove ALL TestWithAI data in {wd}")
    else:
        click.echo(
            "This will remove the TestWithAI Python environment and CLI wrapper.",
        )
    ...
    click.echo("")
    click.echo("TestWithAI uninstalled. Please restart your terminal.")
