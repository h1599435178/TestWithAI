...
def get_available_channels() -> Tuple[str, ...]:
    ...
    test_with_ai.channels), filtered by TEST_WITH_AI_ENABLED_CHANNELS when set.
    """
    from ..app.channels.registry import get_channel_registry

    registry = get_channel_registry()
    all_keys = tuple(registry.keys())
    raw = os.environ.get("TEST_WITH_AI_ENABLED_CHANNELS", "").strip()
    if not raw:
        return all_keys
...
def is_running_in_container() -> bool:
    ...
    Prefer env TEST_WITH_AI_RUNNING_IN_CONTAINER (1/true/yes) at call time so
    supervisord child gets correct value; else check /.dockerenv and cgroup.
    """
    if os.environ.get("TEST_WITH_AI_RUNNING_IN_CONTAINER", "").strip().lower() in (
        "1",
        "true",
        "yes",
    ):
